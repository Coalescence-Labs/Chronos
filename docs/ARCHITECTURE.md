...
| 6 | **Rate limits & caching** | Public-URL viewing + GitHub API both hit limits; caching strategy interacts with #3 and privacy. | **Proposed ADR awaiting ratification:** [adr/006-bff-rate-limits-and-caching.md](adr/006-bff-rate-limits-and-caching.md) — server app token, in-memory 5-min normalized cache, no-auto-retry backoff. Lazy paging (client side) already merged. |
